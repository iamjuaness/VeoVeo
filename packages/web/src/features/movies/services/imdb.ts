...
export async function fetchMoviesFromEndpoint(nextPageToken?: string): Promise<{
  ...
  const url = `${API_URL}titles?types=MOVIE&types=VIDEO&limit=${MOVIES_PER_PAGE}&sortBy=SORT_BY_POPULARITY&sortOrder=ASC&countryCodes=US${nextPageToken ? `&pageToken=${nextPageToken}` : ""
    }`;
  const res = await fetch(url, {
    method: "GET",
    headers: {
