...
export const getGenreDiversity = (movies: any[]) => {
  ...
  // Asumimos 19 géneros estándar (TMDB/IMDb) para calcular el porcentaje de cobertura
  const TOTAL_GENRES = 19;

  return {
    unique: set.size,
    total: movies.length,
    ratio: Math.min(set.size / TOTAL_GENRES, 1),
  };
};
...
export const getGenreAffinity = (movies: any[], minCount = 3) => {
  ...
  movies.forEach((m: MovieWithGenresAndRating) =>
...
export const splitClassicModern = (movies: any[]) => {
  ...
  return {
    ...
    avgClassic: classic.length
      ? classic.reduce((a, m) => a + m.rating, 0) / classic.length
      : 0,
    avgModern: modern.length
      ? modern.reduce((a, m) => a + m.rating, 0) / modern.length
      : 0,
  };
};
...
export const getDurationRatingTrend = (movies: any[]) => {
  ...
  const num = xs
    .map((x, i) => (x - avgX) * (ys[i] - avgY))
    .reduce((a, b) => a + b, 0);
  const denX = Math.sqrt(
    xs.map((x) => (x - avgX) ** 2).reduce((a, b) => a + b, 0)
  );
  const denY = Math.sqrt(
    ys.map((y) => (y - avgY) ** 2).reduce((a, b) => a + b, 0)
  );
...
export function getGenreYearHeatmap(movies: Movie[]) {
  ...
  const decades = Array.from(
    new Set(genres.flatMap((g) => Object.keys(map[g])))
  ).sort();
  return {
    genres,
    decades,
    data: genres.map((g) => ({
      genre: g,
      ...decades.reduce((acc, d) => ({ ...acc, [d]: map[g][d] || 0 }), {}),
    })),
  };
}
